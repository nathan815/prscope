...
export function UserAiSummary({
  ...
  const handleGenerate = useCallback(
    async (skipCache = false) => {
      ...
      try {
        ...
        const r = await generateUserSummary(
          ctx,
          inputPRs,
          { userId, timeRange, fetchLimit },
          skipCache,
        );
        setResult(r);
        setIsStale(false);
  ...
  const thoroughness =
    result?.reviewThoroughness ?? (reviewImpact ? computeReviewThoroughness(reviewImpact) : 0);
        ...
        <div>
          {isStale && result.generatedFor && (
            <div className="flex items-center justify-between bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2 mb-4 text-xs">
              <span className="text-amber-700 dark:text-amber-400">
                Generated for previous selection ({result.generatedFor.timeRange}, limit{" "}
                {result.generatedFor.fetchLimit})
              </span>
              <button
                onClick={() => handleGenerate(true)}
                disabled={loading}
                  ...
                  <div className="space-y-1.5">
                    {ins.topAreas.map((area) => (
                      <div key={area.name} className="flex items-center gap-2 text-xs">
                        <span className="w-52 shrink-0 text-zinc-600 dark:text-zinc-300">
                          {area.name}
                        </span>
                        <div className="flex-1 bg-zinc-100 dark:bg-zinc-800 rounded-full h-4 overflow-hidden">
                          <div
                            className="h-full bg-ado-blue/70 rounded-full transition-all"
                ...
                {ins.workStyle && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-green-600">
                      {ins.workStyle.reviewed}% reviewing
                    </span>
                  </div>
                )}
                {thoroughness > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map((n) => (
                        <div
                          key={n}
                          className={`w-3 h-3 rounded-sm ${n <= Math.round(thoroughness) ? "bg-ado-blue" : "bg-zinc-200 dark:bg-zinc-700"}`}
                        />
                      ))}
                    </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {ins.keyCollaborators.map((name) => (
                      <span
                        key={name}
                        className="text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 px-2.5 py-1 rounded-full"
                      >
                        {name}
                      </span>
                    ))}
          ...
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800 flex-wrap gap-1">
            <span className="text-[11px] text-zinc-400">
              ...
              {format(new Date(result.generatedAt), "MMM d, yyyy h:mm a")}
            </span>
            <button
              onClick={() => handleGenerate(true)}
              disabled={loading}
